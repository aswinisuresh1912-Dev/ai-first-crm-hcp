import os
from typing import Annotated, Sequence, TypedDict, List, Dict, Any, Literal, Optional
...
class LogInteractionInput(BaseModel):
    ...
    date: Optional[str] = Field(default=None, description="Date of interaction. If 'today', resolve to today's date. Format: DD-MM-YYYY or YYYY-MM-DD")
    ...
    materials_shared: Optional[str] = Field(default=None, description="Comma-separated list of materials shared (e.g. 'OncoBoost Phase III PDF, Product X Brochure')")
    samples_distributed: Optional[str] = Field(default=None, description="Comma-separated list of samples distributed (e.g. 'Product X Sample Pack')")
...
class EditInteractionInput(BaseModel):
    ...
    date: Optional[str] = Field(default=None, description="Updated date of interaction")
    ...
    materials_shared: Optional[str] = Field(default=None, description="Updated materials shared list (comma-separated)")
    samples_distributed: Optional[str] = Field(default=None, description="Updated samples distributed list (comma-separated)")
...
class SuggestFollowUpsInput(BaseModel):
    topics_discussed: str = Field(..., description="The topics discussed during the meeting")
    sentiment: str = Field(..., description="The inferred sentiment of the HCP")
...
def get_llm():
    if settings.LLM_PROVIDER == "gemini":
        ...
        try:
            ...
            # Use llama-3.3-70b-versatile which has perfect, native tool-calling support
...
2. **Editing**: When the user requests a correction or update to the form (e.g. "Correction: the name is actually Dr. John"), call 'edit_interaction' to update only the specific fields specified.
...
def _generate_follow_up_suggestions(topics: str, sentiment: str) -> list:
    suggestions = []
    topics_lower = topics.lower() if topics else ""
    
    if "efficacy" in topics_lower or "product x" in topics_lower or "prodo-x" in topics_lower:
        suggestions.append("Schedule follow-up meeting in 2 weeks")
        suggestions.append("Send OncoBoost Phase III PDF")
    if "sharma" in topics_lower or "dr. sharma" in topics_lower:
        suggestions.append("Add Dr. Sharma to advisory board invite list")
    if "cardiologist" in topics_lower or "cardio" in topics_lower:
        suggestions.append("Send CardioShield Clinical Study PDF")
        suggestions.append("Schedule follow-up touchpoint in 1 month")
    
    if not suggestions:
        suggestions.append("Schedule general follow-up in 2 weeks")
        suggestions.append("Send relevant product brochure")
...
def call_tools(state: AgentState):
    ...
    for tool_call in tool_calls:
        ...
        if name == "log_interaction":
            ...
            suggestions = _generate_follow_up_suggestions(topics, sentiment)
