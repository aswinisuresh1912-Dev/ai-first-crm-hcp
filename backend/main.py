...
async def chat_endpoint(payload: Dict[str, Any] = Body(...)):
    ...
    try:
        ...
        err_str = str(e)
        traceback.print_exc()
        # Check for Groq or Gemini rate-limit (429) errors and return a friendly message
        if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "rate_limit_exceeded" in err_str or "RateLimitError" in type(e).__name__:
            # Return 200 with a user-friendly rate-limit message instead of crashing
            return {
                "messages": [{
                    "role": "assistant",
                    "content": "⚠️ **Rate Limit Reached**: The AI model is temporarily unavailable due to API rate limits. Please wait a few seconds and try again — this is a free-tier API limitation, not a bug in the system."
                }],
                "form_data": form_data,
                "last_updated_fields": [],
                "hcp_suggestions": [],
        ...
        raise HTTPException(status_code=500, detail=f"Agent invocation failed: {err_str}")
